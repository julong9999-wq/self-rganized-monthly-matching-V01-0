...
const PortfolioView: React.FC<Props> = ({ portfolio, onUpdateTransaction, onDeleteTransaction, onAddTransaction }) => {
  ...
  const analysisData = useMemo(() => {
    ...
    portfolio.forEach(item => {
        ...
        if (estimatedAnnualDivAmount === 0 && item.etf.dividends && item.etf.dividends.length > 0) {
             ...
             if (latest && latest.amount > 0) {
                 estimatedAnnualDivAmount = totalShares * latest.amount * frequency;
             }
        }
        ...
        if (estimatedAnnualDivAmount > 0) {
            ...
            targetMonths.forEach(m => monthlyDividends[m] += amountPerPeriod);
        }
    ...
    const currentMarketValueLine = Array(12).fill(0).map((_, idx) => {
        ...
        return currentMarketValue + (currentMarketValue * (portfolioAvgReturnRate / 100) * factor);
    });

    let accDiv = 0;
    const totalAssetLine = currentMarketValueLine.map((v, idx) => {
        accDiv += monthlyDividends[idx];
        return v + accDiv;
    });
    ...
    return { 
        monthlyDividends, currentMarketValueLine, totalAssetLine, maxMonthDiv: Math.max(...monthlyDividends), 
        maxY, minY, estimatedAnnualIncome, finalProjectedValue: totalAssetLine[11], portfolioAvgReturnRate, currentMarketValue
    };
            ...
            <div className="pt-2 pb-4 space-y-2">
                ...
                <div className="bg-white p-2 rounded-lg shadow-sm border border-slate-200">
                    <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <div className="flex flex-col"><h4 className="font-bold text-sm text-slate-800">每月預估股息</h4><span className="text-[10px] text-slate-400 leading-none">成本 x 殖利率 (or 配息x頻率)</span></div>
                    </div>
                ...
                <div className="bg-white p-2 rounded-lg shadow-sm border border-slate-200">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex flex-col items-end gap-0.5">
                            <div className="flex items-center gap-1"><div className="w-2 h-0.5 bg-emerald-500 rounded-full"></div><span className="text-[10px] text-slate-500">含息資產</span></div>
                            <div className="flex items-center gap-1"><div className="w-2 h-0.5 bg-blue-400 rounded-full"></div><span className="text-[10px] text-slate-500">市值成長</span></div>
                        </div>
                    </div>
